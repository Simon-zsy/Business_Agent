...
def main():
    ...
    if not jobs:
        print("No jobs returned from JSearch. Check your API key and search parameters.")
        sys.exit(0)
    ...
    if target_companies:
        ...
        if not filtered_jobs:
            print("No jobs matched the company whitelist — showing all results instead")
            filtered_jobs = jobs
    else:
        filtered_jobs = jobs
    ...
    # ------------------------------------------------------------------
    image_url = None
    seedream_key = config.get('seedream_api_key')
    if seedream_key and seedream_key != 'your_seedream_api_key_here':
        image_url = generate_company_bubble_image(
            ranked_jobs,
            seedream_api_key=seedream_key,
            top_n=top_n,
        )
    else:
        print("SEEDREAM_API_KEY not set — skipping bubble image generation")
    ...
    if success:
        ...
        print("Email sending failed, but results are printed above.")
